...
class SCV {
   ...
   public:
    SCV()
        : hp(60),
          attackRate(5)  //* initialization list
    {
        cout << "SCV good to go sir!" << endl;
    }

    SCV(string voiceKor) : hp(60), attackRate(5) { cout << voiceKor << endl; }
...
inline int SCV::getHp() { return hp; }

inline void SCV::setHp(int value) { hp = value; }

inline int SCV::getAttackRate() { return attackRate; }

inline void SCV::setAttackRate(int value) { attackRate = value; }

int main() {
    SCV s1;  // 객체 생성 시점, 생성자가 없다면 컴파일러가 기본 생성자를 만들어
             // 준다. 생성자가 있다면 생성자 함수를 실행한다.
    SCV s2;
